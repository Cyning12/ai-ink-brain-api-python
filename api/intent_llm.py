"""Intent V2：LRU 缓存、LLM 外呼与重试、`decide_intent_v2` 编排。"""
...
if TYPE_CHECKING:
    from .intent_agent import IntentDecision, ToolName, V1Mode

_logger = logging.getLogger("api.intent_agent")
...
def _agent_cache() -> LRUCache:
    from . import intent_agent as ia

    return ia._intent_cache
...
async def _llm_decide_v2_with_retries(
    *,
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            raw_obj, intent_prompts = await ia._llm_decide_v2(
                oai=oai,
                query=query,
                history=history,
                tools=tools,
...
async def decide_intent_v2(
    *,
    ...
    structured = ia.StructuredSignals(
    ...
    def _return_cache_miss(decision: IntentDecision) -> IntentDecision:
        lat = _latency_ms()
        _agent_cache().set(composite_key, _intent_decision_for_cache_store(decision))
    ...
    cached = _agent_cache().get(composite_key)
    if isinstance(cached, ia.IntentDecision):
        return _return_cache_hit(cached)

    try:
        if use_intent_llm:
            oai = ia.openai_siliconflow_client()
            if not use_tools:
            ...
            decision2 = ia.apply_hints_arbitration(decision2, query=query)
            return _return_cache_miss(decision2)
    except asyncio.TimeoutError:
        v1 = ia.decide_intent_v1(query=query, prefer="auto")
