...
# -*- coding: utf-8 -*-
"""
闸口 C S0：对指定 task 跑 CTX_V2_QUERY / CTX_DUAL_MD（默认两臂）。

臂 D：v2 graph_query 子图 JSON；臂 E：精选双轨 Markdown（物化 dual_track）。
"""
...
def gate_c_arm_specs(task_id: str) -> dict[str, tuple[str, Path]]:
    """臂 → (主载荷标签, 物化文件路径)。"""
    return {
        "CTX_V2_QUERY": (
            f"v2 graph_query 子图（task={task_id}）",
            FIXTURE_ROOT / "payloads" / "CTX_V2_QUERY" / f"{task_id}.subgraph.json",
        ),
        "CTX_DUAL_MD": (
            f"精选双轨原文（task={task_id}）",
            FIXTURE_ROOT / "payloads" / "CTX_DUAL_MD" / f"{task_id}.dual_track.md",
        ),
    }


def _dry_stub_response() -> dict[str, Any]:
    return {
        "entrypoints": [],
        "impacts": [],
        "evidence": [],
...
def execute_gate_c_s0(
    *,
    ...
    if not model_resolved and not dry_run:
        raise RuntimeError("protocol 中缺少 model")
    temperature = float(protocol.get("temperature") or 0.2)
    max_tokens = int(protocol.get("max_tokens") or 4096)
    protocol_version = str(protocol.get("protocol_version") or "gate_ctx_c_v1")
    freeze_id = str(protocol.get("freeze_id") or "")

    specs = gate_c_arm_specs(task_id)
    for arm in arms:
        if arm not in specs:
            raise ValueError(f"未知臂 {arm}")
        path = specs[arm][1]
        if not path.is_file():
            raise FileNotFoundError(f"缺少载荷 {path}；先运行 materialize_gate_c_payloads.py")
    ...
    if dry_run:
        for i, (arm, label, text) in enumerate(arm_items, start=1):
            stub = _dry_stub_response()
            record = {
                "schema": "gate_ctx_c_s0_record_v1",
                "run_id": run_id,
                "arm": arm,
                "call_index": i,
                "arms_order": arms,
                ...
                "response": stub,
                "main_payload_bytes": len(text.encode("utf-8")),
                "main_label": label,
            }
            summary = {
                "call_index": i,
    ...
    for arm in arms:
        ...
        record["schema"] = "gate_ctx_c_s0_record_v1"
    ...
    index = {
        "schema": "gate_ctx_c_run_index_v1",
        "run_id": run_id,
        "task_id": task_id,
        "segment": "S0",
        "protocol_version": protocol_version,
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="gate_ctx_c_v1 S0")
    p.add_argument("--task-id", required=True)
    p.add_argument(
        "--arms",
        default="CTX_V2_QUERY,CTX_DUAL_MD",
        help="逗号分隔臂，默认 D+E",
    )
    ...
    p.add_argument("--dry-run", action="store_true", help="不写 LLM，仅落盘结构")
    return p.parse_args()
...
def main() -> int:
    ...
    run_dir = RUNS_ROOT / f"gate_ctx_c_v1_{args.task_id}_{ts}"
    try:
        _, _, code = execute_gate_c_s0(
            run_dir=run_dir,
