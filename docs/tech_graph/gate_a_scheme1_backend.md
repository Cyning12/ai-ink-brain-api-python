...
### 主结论口径（本次更新 · 2026-05-15）

**范围声明**：此次方案1 与闸口附录交付 **未** 在用户可见页面中引入 **大图谱 Mermaid 运行时渲染**；**主结论以 Agent / LLM 消费侧为准**（理解框架、边界控制、查找与影响分析的效率），而非以浏览器首屏为必达指标。

**价值方向（可随项目扩充）**

1. **更少上下文即可理解项目**：依赖与模块关系由 **`graph.json`** 与既有 `docs/_tech_graph` 规约集中承载，Agent 不必在长文 Mermaid 源中自行抽结构。  
2. **增强 AI 对项目的掌控力、减少漂移**：**`--check`** 与 CI 将 **`.ai.md` 与 `graph.json` 绑定**；修改前可读图做 **影响范围** 判断，降低无意义大改或改不全。  
3. **降低 token 使用**：在同等拓扑信息需求下，优先向模型提供 **`graph.json`** 等 **短、结构化** 输入；附录 **`tech_graph_token_estimate.py`** 提供与 Mermaid 拼接体的 **同规则粗估**对照（非厂商官方 tokenizer）。

**与 §3.2 / §5 的关系**：**§3.2（浏览器包体、LCP、首帧等）** 在本阶段记为 **不适用（N/A）** 于主结论；**§5** 中针对「首帧 / gzip」等 **浏览器向** 阈值 **不**自动用于签收本阶段——若未来产品上站大图谱 Mermaid，再启用 §3.2 并修订本段。

- **是否进入方案2 筹备**：**暂缓**（仍遵守工作区规划 **R2**：须先有闸口 A/B 书面结论与方案2 单独立项）。**本阶段**在 **Agent/LLM 口径**下已形成可引用的 **附录证据链**（§3.1、`token_estimate`、CI）；**不**因 §3.2 为 N/A 而阻塞对「静态 `graph.json` 作为 Agent 主输入」的采纳。  
- **理由（补充）**：方案1 已满足「确定性解析 + 无漂移门禁」；在 **无用户页 Mermaid 大图** 的前提下，继续纠结 **浏览器体验向**对比易偏离本轮目标；方案2 仍应在 **R2** 与后续闸口就绪后再动。
...
**本阶段默认**：**不适用（N/A）** —— 产品上 **无**用户页大图谱 Mermaid 渲染需求时，**不必**为闸口主结论补齐下表；若未来启用，再按表采集并删除本段 N/A 声明。
...
**适用说明**：下表 **「P95 解析 / 首帧 / gzip」** 句式主要针对 **§3.2 已启用（浏览器）** 的情形；**本阶段 §3.2 为 N/A** 时，**不**将下列浏览器阈值作为 Agent 口径签收条件；Agent 向可参照 **§3** 中 **载荷 / token 附录** 与 §3.1 耗时自洽说明。
