...
```text
你正在执行 **RECENT_TASK_SCHEDULE P0-A2**：在 `docs/harness/prompts/HANDOFF_SEMI_AUTO.md` 增补 **Harness 状态栏（版本 B）**（每棒结束须输出）。
...
1. 在 `HANDOFF_SEMI_AUTO.md` 新增 §「每棒状态栏（版本 B）」：多行结构化字段（当前帽、audit_profile、human_gate 摘要、下一棒 A/B、分支名）。
