...
- docs/harness/prompts/HANDOFF_AUTO_COMMIT.md
...
2. **40 自检帽**（semi_auto 同会话切换）：
   - 落盘 invoke：`docs/harness/invokes/invoke_20260523_40_chatbi-v3-p2-resilience-spec.md`
   - 跑 pytest；回填 task **`### 自检结论（执行者）`**（退出码、passed 数、纯 docs 说明）
   - commit 本轮路径（HANDOFF_AUTO_COMMIT）

3. **50 独立复检帽**（**不可省略**，即使 HG-REINSPECT 已预批）：
   - 落盘 invoke：`docs/harness/invokes/invoke_20260523_50_chatbi-v3-p2-resilience-spec.md`
