...
- 变更范围说明（无则写「无」）：
git diff 034fdcd..f803f87 — 限流 middleware、test_rate_limit_routes、PROJECT_CONFIG、task 自检
...
3. 将 **`### 自检结论（执行者）`** 写入 **task 路径** 指向的 task 正文（若 30 已填则复核/增补，勿删除已有证据）；含命令列表、退出码、验收摘要、已知未测项。
