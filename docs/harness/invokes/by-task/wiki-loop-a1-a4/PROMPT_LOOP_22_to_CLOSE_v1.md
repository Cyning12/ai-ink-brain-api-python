...
> **invoke C2**：见 [`SKILL-harness-loop-batch`](../../../tasks/skills/SKILL-harness-loop-batch.md) §invoke 质量门禁；§3 步骤 1–5 **换帽前**须自检（A2+ 与 A1 同标准）。
...
- **invoke C2（硬）**：每帽 invoke §3 全文 ≥15 行（或文件 ≥800B）；元信息表含 round/hat/task/task_slug/freeze_id/git_branch；cross-round 续跑 A2+ **与 A1 同标准**（见 SKILL-harness-loop-batch §invoke 质量门禁）
...
- 落盘 review：docs/harness/reviews/by-task/wiki-loop-a1-a4/task_<basename>_audit_R1_YYYYMMDD.md
- 无阻塞 → 准许 30
...
【开帽】落盘 invoke：
docs/harness/invokes/by-task/wiki-loop-a1-a4/invoke_YYYYMMDD_30_{{TASK_SLUG}}-v1.md
...
【开帽】落盘 invoke：
docs/harness/invokes/by-task/wiki-loop-a1-a4/invoke_YYYYMMDD_40_{{TASK_SLUG}}-v1.md
...
【开帽】落盘 invoke：
docs/harness/invokes/by-task/wiki-loop-a1-a4/invoke_YYYYMMDD_50_{{TASK_SLUG}}-v1.md
...
【开帽】落盘 invoke：
docs/harness/invokes/by-task/wiki-loop-a1-a4/invoke_YYYYMMDD_CLOSE_{{TASK_SLUG}}-v1.md
