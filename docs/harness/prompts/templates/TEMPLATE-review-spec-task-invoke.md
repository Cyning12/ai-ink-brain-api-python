...
> **帽子真值**：[`hats/20-review-spec-task.md`](hats/20-review-spec-task.md)（与 [`hats/22-task-audit.md`](hats/22-task-audit.md) 分工）。  
> **字段约定**：[`../HARNESS_V2_PLAN.md`](../HARNESS_V2_PLAN.md) **§5**。  
> **需要书面闭环、签收 / 关闭**：改用 [`templates/TEMPLATE-task-audit-invoke.md`](templates/TEMPLATE-task-audit-invoke.md) + [`hats/22-task-audit.md`](hats/22-task-audit.md)。
...
- docs/harness/prompts/hats/20-review-spec-task.md（身份、只做什么、禁止什么；不要求默认落盘 reviews）
...
7. **自动 commit**：在输出下一棒 Prompt 后，若本轮有 invoke 等磁盘变更，按 docs/harness/prompts/handoff/HANDOFF_AUTO_COMMIT.md 分仓 commit；仅对话、零文件变更则不必空提交。用户写明「不要 commit」则跳过。

若用户明确要求落盘审查文档，应提示其改用 docs/harness/prompts/templates/TEMPLATE-task-audit-invoke.md（任务审核帽 22）。
```
...
`Harness`、`templates/TEMPLATE-review-spec-task-invoke`、`20-review-spec-task`、`可测性`、`failure_paths`、`22-task-audit`、`追问`
