...
> **定位**：在 [`templates/TEMPLATE-independent-reinspect-invoke.md`](templates/TEMPLATE-independent-reinspect-invoke.md) **§3 短版**之上的 **v2 全量**可复制 Prompt：补齐 **模式前置检查**、**`failure_paths` 逐项**、**`test_strategy` 专节**、**按子仓拆分的必绿 checklist 叙事**、**PR 验收小结** 与 **停止条件**。  
> **真值**：[`hats/50-independent-reinspect.md`](hats/50-independent-reinspect.md)；字段细则 [`../HARNESS_V2_PLAN.md`](../HARNESS_V2_PLAN.md) §5。  
...
真值文档：docs/harness/prompts/hats/50-independent-reinspect.md
══════════════════════════════════════════════════════════════════
...
【二、强制遵循（阅读顺序）】
1. docs/harness/prompts/hats/50-independent-reinspect.md（§一 / §二 边界、禁止项、交接物）
...
9. **自动 commit**：在输出下一棒 Prompt 且本轮复检/验收报告已落盘后，按 docs/harness/prompts/handoff/HANDOFF_AUTO_COMMIT.md 分仓 commit。仅对话、零文件变更则不必空提交；用户写明「不要 commit」则跳过。
```
...
`Harness`、`50-independent-reinspect`、`templates/TEMPLATE-independent-reinspect-invoke`、`templates/TEMPLATE-independent-reinspect-invoke-full`、`复检`、`全局验收`、`两者`、`占位符`、`EXAMPLE-invoke`
