...
## 5. 三轮完成后 · 下一棒（10 帽 · 人择 A/B）

> **三轮（轮 0→1→2）解决的是 SPEC/task 怎么写准**；**不**替代 Harness 的 **22 vs 30** 分工。

```text
轮 0～2 完成 + SPEC 待确认清单已全部「人确认」
    + task 草案（验收 / failure_paths / test_strategy / freeze_id）齐
        ↓
10 帽结束：仍须输出 路径 A（22）与 路径 B（30）全文 — 由人择一
        ↓
    ┌─ 人择 A → 22 R1 无阻塞 → 30
    └─ 人择 B → 30（跳过 22；人承担闸 1，见 10-requirements §下一棒 A/B）
```

| 问题 | 答案 |
| --- | --- |
| 走完三轮能否 **直接 30**？ | **可以**，当且仅当：① 清单无 pending；② task 可执行；③ **人明确选择路径 B**（或 task 正文显式推荐 B 且你已确认）。 |
| 走完三轮是否 **默认跳过 22**？ | **否**。`audit_profile: post_close` / `test_strategy: required` 等仍 **推荐 A**（见 `10-requirements.md` §下一棒 A/B）。 |
| 30 中发现 SPEC 缺口？ | **停工**回轮 2 或开 20；禁止在实现 PR 中偷偷改 SPEC。 |

**与 Wiki / 实验**：`governance/SPEC-*` 或 §3 省略类 task **不走** L1 全套三轮，但仍走 10 帽 task + A/B。
...
| 2026-05-25 | v1.1：§5 三轮完成后与 22/30 下一棒关系 |
