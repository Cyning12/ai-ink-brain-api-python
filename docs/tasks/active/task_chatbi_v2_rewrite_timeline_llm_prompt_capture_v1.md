...
- [ ] `CHATBI_USE_AGENT=true`，走 RAG：SSE 中在 `rag_search` 的 `tool.call.start` 之前出现 **`rag.rewrite`** 的 start/end，且 end 含 **`rewritten_query`**。  
- [ ] 开启 `debug_llm_prompts`（body 或 env）：至少出现 **`agent.debug.llm_prompts`**（Intent 一次；若工具链有多段 LLM，可按 step 多次），`items[].messages` 为发往模型的完整消息列表。  
- [ ] `pytest tests/test_intent_cache.py tests/test_unified_chat_backend_v2_agent.py tests/test_unified_chat_sse_incremental_vnext.py` 通过。
