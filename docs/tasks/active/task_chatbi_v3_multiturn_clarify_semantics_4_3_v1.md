...
> **0–4 还要不要再走一遍？**：若 **首包 / 已合并 PR** 里已经把读规、契约、`contract_check`、pytest、日志 `run_id`、RBAC 声明都做过，**后续小改**（例如只调 env 注释、修文案）**不必**从零重跑 0–4 全表；只要 **关单前浓缩核对** 里与本次改动相关的行仍为真。若 PR **又动**了契约、Agent 澄清分支或话术露表名，则 **只补跑** 对应阶段即可。
...
### 阶段 5 — 架构图谱 `_tech_graph/`（≠ 日志；≠ contract manifest）

**一句话**：阶段 5 管的是 **`docs/_tech_graph/`** 里 **Mermaid 流程图**（架构说明），用来让 **「请求 → Agent → 澄清 / Text2SQL」的控制流** 和 **代码** 对得上；**不是** `CHATBI_JSON_LOG`、**也不是** `_contract_manifest.json`（那归 **阶段 1**）。

| 步骤 | 何时要做 | 对应验收 | 最小产出 |
|------|----------|----------|----------|
| **5.1** | 本 PR **改了** Agent **控制流**（例：`api/agent.py` 里澄清门控、短路 tool、与 SSE emit 顺序相关的逻辑），图上的「边」应有新语义 | **图谱** | 更新 **`docs/_tech_graph/`** 中相关 **`11_flow_*` / `00_main`** 等：人类版 **`.md`**；若含 **flowchart** 还须按仓规维护 **`.ai.md`** 双轨 |
| **5.2** | 本 PR **只**动 env 默认值、注释、与流程无关的单测等，**流程图不需要新节点/新边** | **图谱** | **§实现备忘** 写 **`图谱：N/A`**，**不必**为改而改图 |
