...
- [x] **G-W3-1** 在 [`PROJECT_CONFIG_AI_INK_BRAIN_API_PYTHON.md`](../meta/PROJECT_CONFIG_AI_INK_BRAIN_API_PYTHON.md) **§C** 增补 **portfolio 演示站** 段落：`CONTENT_ROOT`、**`SYNC_ADMIN_SECRET`**（admin/sync 真值）、`EMBEDDING_DIM`、`SILICONFLOW_*`、Supabase 写库变量（**不含真实密钥**）  
