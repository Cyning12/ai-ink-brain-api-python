...
- [ ] **新增落库字段**：写入 `public.rag_conversation_logs.metadata.router_debug.router_trace_v1.text2sql_exec`
- [ ] **触发条件**：仅当最终 `mode == "text2sql"` 且实际执行了 SQL 时写入
- [ ] **数据约束**：必须截断/限量/脱敏，禁止落库完整结果集与大字段
- [ ] **体积约束**：与 `router_trace_v1` 的 8KB shrink 策略兼容；超限时优先丢弃 `rows_preview`
- [ ] **测试**：新增最小 pytest 覆盖（mock SQL 执行结果），验证字段存在与截断策略生效
