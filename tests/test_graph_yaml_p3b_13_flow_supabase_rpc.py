"""Tests for 13_flow_supabase_rpc.graph.yaml — YAML→MD conversion + diff check."""
...
GRAPH_YAML = REPO_ROOT / "docs" / "_tech_graph" / "13_flow_supabase_rpc.graph.yaml"
GRAPH_JSON = REPO_ROOT / "docs" / "_tech_graph" / "graph.json"
COMPILE_SCRIPT = REPO_ROOT / "scripts" / "graph_yaml_compile.py"


class TestGraphYamlP3b13FlowSupabaseRpc:
    """P3b: 13_flow_supabase_rpc YAML source validation."""

    def test_13_flow_supabase_rpc_yaml_source_exists(self):
        """F1: YAML source must exist."""
        assert GRAPH_YAML.exists(), f"YAML source not found: {GRAPH_YAML}"

    def test_13_flow_supabase_rpc_yaml_source_is_valid_yaml(self):
        ...
        assert data.get("graph_id") == "13_flow_supabase_rpc", "YAML graph_id must be 13_flow_supabase_rpc"
        assert "nodes" in data, "YAML must have nodes"
        assert "edges" in data, "YAML must have edges"

    def test_13_flow_supabase_rpc_yaml_nodes_match_graph_json(self):
        """F2: YAML nodes must match graph.json 13_flow_supabase_rpc slice."""
        ...
        json_nodes = {
            ...
            if n.get("graph_id") == "13_flow_supabase_rpc"
        }
    ...
    def test_13_flow_supabase_rpc_yaml_edges_match_graph_json(self):
        """F2: YAML edges must match graph.json 13_flow_supabase_rpc slice."""
        if not GRAPH_YAML.exists() or not GRAPH_JSON.exists():
            pytest.skip("Source files not found")
        ...
        json_edges = [
            ...
            if e.get("graph_id") == "13_flow_supabase_rpc" and "from" in e and "to" in e
        ]
        json_edge_keys = {
    ...
    def test_13_flow_supabase_rpc_yaml_anchors_match_graph_json(self):
        ...
        json_edges = [
            e
            for e in json_data.get("edges", [])
            if e.get("graph_id") == "13_flow_supabase_rpc" and "from" in e
        ]
    ...
    def test_13_flow_supabase_rpc_compile_script_check_mode(self):
        """F2: Compile script --check must exit 0 for 13_flow_supabase_rpc."""
        if not COMPILE_SCRIPT.exists():
            pytest.skip("Compile script not yet created")
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--check", "--graph-id", "13_flow_supabase_rpc"],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_13_flow_supabase_rpc_anchor_format_in_generated_mermaid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            tmp_path = f.name
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--graph-id", "13_flow_supabase_rpc", "--output", tmp_path],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_13_flow_supabase_rpc_ai_md_deprecated(self):
        """F3: .ai.md must be marked deprecated."""
        ai_md = REPO_ROOT / "docs" / "_tech_graph" / "13_flow_supabase_rpc.ai.md"
