"""Tests for 10_flow_rag.graph.yaml — YAML→MD conversion + diff check."""
...
GRAPH_YAML = REPO_ROOT / "docs" / "_tech_graph" / "10_flow_rag.graph.yaml"
GRAPH_JSON = REPO_ROOT / "docs" / "_tech_graph" / "graph.json"
COMPILE_SCRIPT = REPO_ROOT / "scripts" / "graph_yaml_compile.py"


class TestGraphYamlP110FlowRag:
    """P1: 10_flow_rag YAML source validation."""

    def test_10_flow_rag_yaml_source_exists(self):
        """F1: YAML source must exist."""
        assert GRAPH_YAML.exists(), f"YAML source not found: {GRAPH_YAML}"

    def test_10_flow_rag_yaml_source_is_valid_yaml(self):
        ...
        assert data.get("graph_id") == "10_flow_rag", "YAML graph_id must be 10_flow_rag"
        assert "nodes" in data, "YAML must have nodes"
        assert "edges" in data, "YAML must have edges"

    def test_10_flow_rag_yaml_nodes_match_graph_json(self):
        """F2: YAML nodes must match graph.json 10_flow_rag slice."""
    ...
    def test_10_flow_rag_yaml_edges_match_graph_json(self):
        """F2: YAML edges must match graph.json 10_flow_rag slice."""
        if not GRAPH_YAML.exists() or not GRAPH_JSON.exists():
            pytest.skip("Source files not found")
        ...
        json_edges = [
            ...
            if e.get("graph_id") == "10_flow_rag" and "from" in e and "to" in e
        ]
        json_edge_keys = {
    ...
    def test_10_flow_rag_yaml_anchors_match_graph_json(self):
        ...
        json_edges = [
            e
            for e in json_data.get("edges", [])
            if e.get("graph_id") == "10_flow_rag" and "from" in e
        ]
    ...
    def test_10_flow_rag_compile_script_check_mode(self):
        """F2: Compile script --check must exit 0 for 10_flow_rag."""
        if not COMPILE_SCRIPT.exists():
            pytest.skip("Compile script not yet created")
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--check", "--graph-id", "10_flow_rag"],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_10_flow_rag_anchor_format_in_generated_mermaid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            tmp_path = f.name
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--graph-id", "10_flow_rag", "--output", tmp_path],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_10_flow_rag_ai_md_deprecated(self):
        """F3: .ai.md must be marked deprecated."""
        ai_md = REPO_ROOT / "docs" / "_tech_graph" / "10_flow_rag.ai.md"
