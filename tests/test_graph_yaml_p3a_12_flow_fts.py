"""Tests for 12_flow_fts.graph.yaml — YAML→MD conversion + diff check."""
...
GRAPH_YAML = REPO_ROOT / "docs" / "_tech_graph" / "12_flow_fts.graph.yaml"
GRAPH_JSON = REPO_ROOT / "docs" / "_tech_graph" / "graph.json"
COMPILE_SCRIPT = REPO_ROOT / "scripts" / "graph_yaml_compile.py"


class TestGraphYamlP3a12FlowFts:
    """P3a: 12_flow_fts YAML source validation."""

    def test_12_flow_fts_yaml_source_exists(self):
        """F1: YAML source must exist."""
        assert GRAPH_YAML.exists(), f"YAML source not found: {GRAPH_YAML}"

    def test_12_flow_fts_yaml_source_is_valid_yaml(self):
        ...
        assert data.get("graph_id") == "12_flow_fts", "YAML graph_id must be 12_flow_fts"
        assert "nodes" in data, "YAML must have nodes"
        assert "edges" in data, "YAML must have edges"

    def test_12_flow_fts_yaml_nodes_match_graph_json(self):
        """F2: YAML nodes must match graph.json 12_flow_fts slice."""
    ...
    def test_12_flow_fts_yaml_edges_match_graph_json(self):
        """F2: YAML edges must match graph.json 12_flow_fts slice."""
        if not GRAPH_YAML.exists() or not GRAPH_JSON.exists():
            pytest.skip("Source files not found")
        ...
        json_edges = [
            ...
            if e.get("graph_id") == "12_flow_fts" and "from" in e and "to" in e
        ]
        json_edge_keys = {
    ...
    def test_12_flow_fts_yaml_anchors_match_graph_json(self):
        ...
        json_edges = [
            e
            for e in json_data.get("edges", [])
            if e.get("graph_id") == "12_flow_fts" and "from" in e
        ]
    ...
    def test_12_flow_fts_compile_script_check_mode(self):
        """F2: Compile script --check must exit 0 for 12_flow_fts."""
        if not COMPILE_SCRIPT.exists():
            pytest.skip("Compile script not yet created")
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--check", "--graph-id", "12_flow_fts"],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_12_flow_fts_anchor_format_in_generated_mermaid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            tmp_path = f.name
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--graph-id", "12_flow_fts", "--output", tmp_path],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_12_flow_fts_ai_md_deprecated(self):
        """F3: .ai.md must be marked deprecated."""
        ai_md = REPO_ROOT / "docs" / "_tech_graph" / "12_flow_fts.ai.md"
