"""Tests for 15_e2e_boundary.graph.yaml — YAML→MD conversion + diff check."""
...
GRAPH_YAML = REPO_ROOT / "docs" / "_tech_graph" / "15_e2e_boundary.graph.yaml"
GRAPH_JSON = REPO_ROOT / "docs" / "_tech_graph" / "graph.json"
COMPILE_SCRIPT = REPO_ROOT / "scripts" / "graph_yaml_compile.py"


class TestGraphYamlP515E2eBoundary:
    """P5: 15_e2e_boundary YAML source validation."""

    def test_15_e2e_boundary_yaml_source_exists(self):
        """F1: YAML source must exist."""
        assert GRAPH_YAML.exists(), f"YAML source not found: {GRAPH_YAML}"

    def test_15_e2e_boundary_yaml_source_is_valid_yaml(self):
        ...
        assert data.get("graph_id") == "15_e2e_boundary", "YAML graph_id must be 15_e2e_boundary"
        assert "nodes" in data, "YAML must have nodes"
        assert "edges" in data, "YAML must have edges"

    def test_15_e2e_boundary_yaml_nodes_match_graph_json(self):
        """F2: YAML nodes must match graph.json 15_e2e_boundary slice."""
        ...
        json_nodes = {
            ...
            if n.get("graph_id") == "15_e2e_boundary"
        }
    ...
    def test_15_e2e_boundary_yaml_edges_match_graph_json(self):
        """F2: YAML edges must match graph.json 15_e2e_boundary slice."""
        if not GRAPH_YAML.exists() or not GRAPH_JSON.exists():
            pytest.skip("Source files not found")
        ...
        json_edges = [
            ...
            if e.get("graph_id") == "15_e2e_boundary" and "from" in e and "to" in e
        ]
        json_edge_keys = {
    ...
    def test_15_e2e_boundary_yaml_anchors_match_graph_json(self):
        ...
        json_edges = [
            e
            for e in json_data.get("edges", [])
            if e.get("graph_id") == "15_e2e_boundary" and "from" in e
        ]
    ...
    def test_15_e2e_boundary_compile_script_check_mode(self):
        """F2: Compile script --check must exit 0 for 15_e2e_boundary."""
        if not COMPILE_SCRIPT.exists():
            pytest.skip("Compile script not yet created")
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--check", "--graph-id", "15_e2e_boundary"],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_15_e2e_boundary_anchor_format_in_generated_mermaid(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
            tmp_path = f.name
        result = subprocess.run(
            [sys.executable, str(COMPILE_SCRIPT), "--graph-id", "15_e2e_boundary", "--output", tmp_path],
            capture_output=True,
            text=True,
            cwd=REPO_ROOT,
        )
    ...
    def test_15_e2e_boundary_ai_md_deprecated(self):
        """F3: .ai.md must be marked deprecated."""
        ai_md = REPO_ROOT / "docs" / "_tech_graph" / "15_e2e_boundary.ai.md"
