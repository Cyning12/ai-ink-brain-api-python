...
def test_doc_review_stub_paste(lab_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    # 本地 dev 无 legacy 密钥时 deps 放行 maintainer
...
def test_doc_review_rejects_path_not_in_allowlist(lab_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("api.ops.deps.legacy_secret_candidates", lambda: [])
    res = lab_client.post(
        "/api/py/agently-lab/doc-review",
        json={"profile": "task-audit", "target_path": "../secret.env"},
    )
    assert res.status_code == 400
    body = res.json()
    assert body["detail"]["code"] == "DOC_REVIEW_INVALID"


def test_doc_review_rejects_non_maintainer(lab_client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    # 让 legacy_secret_candidates 返回非空，使非 maintainer 被拒绝
    monkeypatch.setattr("api.ops.deps.legacy_secret_candidates", lambda: ["some-secret"])
    res = lab_client.post(
        "/api/py/agently-lab/doc-review",
        json={"profile": "task-audit", "content": "# Task\n"},
        headers={"x-ops-legacy-secret": "wrong-secret"},
    )
