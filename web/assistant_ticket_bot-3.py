...
def init_agent_service():
    ...
    try:
        try:
            ...
            print("助手初始化成功！", flush=True)
            return bot
        except Exception as e:
            msg = str(e)
            if 'Docker' in msg or 'docker' in msg:
                print(f"检测到 Docker 不可用，已降级禁用 code_interpreter: {msg}", flush=True)
                bot = Assistant(
                    llm=llm_cfg,
                    name='门票助手',
                    description='门票查询与订单分析',
                ...
                print("助手初始化成功！（降级模式）", flush=True)
                return bot
            raise
    except Exception as e:
        print(f"助手初始化失败: {str(e)}", flush=True)
        raise
...
def app_gui():
    ...
    try:
        ...
        print("Web 界面准备就绪，正在启动服务...", flush=True)
        # 启动 Web 界面
        WebUI(
            bot,
        ...
        print(f"启动 Web 界面失败: {str(e)}", flush=True)
        print("请检查网络连接和 API Key 配置", flush=True)
